...
try:
    import requests  # đảm bảo đã pip install "requests[socks]"
except Exception:
...
def parse_junit(path: str):
    ...
    for s in suites:
        ...
        try: summary["time"] += float(s.attrib.get("time", 0.0))
        except Exception: pass
        for tc in s.findall("testcase"):
            name = tc.attrib.get("name", "")
            cls  = tc.attrib.get("classname", "")
            full = f"{cls}::{name}" if cls else name
            fail = tc.find("failure"); err  = tc.find("error"); skp  = tc.find("skipped")
            if fail is not None:
